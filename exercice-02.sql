...
-- - nom : SASS
-- - description : aucune
INSERT INTO tag (name) value ('SASS');
...
WHERE student.id = 1 AND tag.name = 'SASS';

-- Exo 2.4
-- Mettez à jour le tag `HTML` :
-- description : Le HTML est un language de structuration de données.
UPDATE tag SET description = 'Le HTML est un langage de structuration de données.' WHERE id = 1;
...
WHERE student_tag.student_id = (SELECT id FROM student WHERE student.firstname = 'Zacharie' AND student.lastname ='Evrard') AND student_tag.tag_id = (SELECT id FROM tag WHERE tag.name = 'Terminal Bash');
...
SELECT * FROM student
WHERE student.id = (SELECT id FROM student WHERE student.firstname = 'Arthur' AND student.lastname = 'Lacombe');

-- Exo 2.8
-- Supprimez toutes les relations entre un student et ses tags :
-- - student : Odette Thomas
DELETE FROM student_tag
WHERE student_tag.student_id = (SELECT id FROM student WHERE student.firstname = 'Odette' AND student.lastname = 'Thomas');
...
ALTER TABLE student DROP FOREIGN KEY "IDX_B723AF33166D1F9C",
ALTER TABLE project_tag DROP FOREIGN KEY "IDX_91F26D60166D1F9C"
DELETE FROM project WHERE `name` = 'Ut delectus explicabo.'